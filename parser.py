...
#
# FIXES applied:
#   1. UNIT_PC, UNIT_S, UNIT_MAG removed from p_unit grammar rule
#      to match updated lexer which no longer reserves 'pc','s','mag'.
#   2. No other changes from the previously working version.
...
def p_func_def_inner(p):
    ...
    p[0] = FuncDef(p[2], p[4], p[7], ret, lineno=_ln(p))
...
# ── Units ─────────────────────────────────────────────────────────────────────
# UNIT_PC, UNIT_S, UNIT_MAG removed - no longer in lexer reserved words
def p_unit(p):
    '''unit : UNIT_KM
            | UNIT_DEG
            | UNIT_ARCSEC
